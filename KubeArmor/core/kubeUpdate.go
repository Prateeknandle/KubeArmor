		...
		cache.ResourceEventHandlerFuncs{
			...
			UpdateFunc: func(oldObj, newObj interface{}) {
				if policy, ok := newObj.(*ksp.KubeArmorClusterPolicy); ok {
					...
					dm.Logger.Printf("Detected a Cluster Security Policy (modified/%s/%s)", secPolicy.Metadata["namespaceName"], secPolicy.Metadata["policyName"])
			...
			DeleteFunc: func(obj interface{}) {
				if policy, ok := obj.(*ksp.KubeArmorClusterPolicy); ok {
					...
					dm.Logger.Printf("Detected a Cluster Security Policy (deleted/%s/%s)", secPolicy.Metadata["namespaceName"], secPolicy.Metadata["policyName"])
